...
const AdminDashboard = () => {
  ...
  const updateStatus = async (id, newStatus) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/issues/admin/issues/${id}/status`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        ...
        setIssues((prev) =>
          prev.map((issue) => (issue._id === id ? updated : issue))
        );
  ...
  const getPriorityCount = (priority) =>
    issues.filter(
      (issue) => getPriorityFromUpvotes(issue.upvotes?.length || 0) === priority
    ).length;
  ...
  const getMonthlyTrends = () => {
    ...
    issues.forEach((issue) => {
      const key = dayjs(issue.createdAt || issue.dateReported).format(
        "YYYY-MM"
      );
  ...
  const filteredIssues =
    filteredPriority === "All"
      ...
      : filteredIssuesByStatus.filter(
          (issue) =>
            getPriorityFromUpvotes(issue.upvotes?.length || 0) ===
            filteredPriority
        );
